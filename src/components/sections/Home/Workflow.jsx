...
export default function Workflow() {
  return (
    <>
      <section
        id="layanan"
        className="pt-16 pb-14 lg:pt-24 lg:pb-24 bg-[#FAF8F5] text-[#1F2D3D]"
      >
      ...
      <section
        id="workflow"
        className="relative overflow-hidden bg-[#1F2D3D] text-[#FAF8F5] py-16 lg:py-32 px-5 md:px-12"
      >
        <div
              ...
              {PROCESS_STEPS.map((step, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={`w-full md:w-1/2 ${isEven ? "md:pr-16 md:text-right md:ml-0" : "md:ml-auto md:pl-16"}`}
                    >
                      ...
                      <div
                        className={`hidden md:block bg-white/5 border border-white/8 rounded-xl p-6 lg:p-8 backdrop-blur-sm ${isEven ? "md:mr-2" : "md:ml-2"}`}
                      >
                        <h3 className="text-xl lg:text-2xl font-serif tracking-wider uppercase text-[#FAF8F5] mb-2">
                          {step.title}
                        </h3>
                        <p className="text-xs md:text-base font-sans text-[#FAF8F5]/65 leading-relaxed">{step.desc}</p>
                      </div>
                    </div>
                  </motion.div>
                );
